...
public class CassandraQueryResponse {
    ...
    private static String host = "timcowley.com";
    ...
    /**
     * ML USECASE Returns a List of Providers which have an average submitted
     * charge amount either above or below the given cost
     * 
     * @author Tim
     * 
     * @param code
    ...
    /**
     * Returns the average cost of a procedure in a given state
     * 
     * @author Tim, others
     * 
     * @param state
     *            a state represented as standard upper case 2 letter
     *            abbreviation
     * @param code
     *            hcpcs_code
     * @return a Double representing the averaging of the average submitted
     *         charge amount, if no result returns -1, if query fails returns 0
     */
    public static Double getAverage(String state, String code) {
        ...
        try {
            ...
            Statement selectStmt = QueryBuilder.select("average_submitted_chrg_amt")
                    .from("mv_procedures_state_cost_2012")
                    .where(QueryBuilder.eq("nppes_provider_state", state))
                    .and(QueryBuilder.eq("hcpcs_code", " " + code.toUpperCase()));
            // FIXME there are some mistakes that require additional formating
            // of hcpcs_code
            // to make this work properly
            ResultSet result = session.execute(selectStmt);
            if (result.getAvailableWithoutFetching() == 0) {
                average = -1.0;
            } else {
                average = (double) result.one().getFloat("average_submitted_chrg_amt");
            }
        } catch (Exception e) {
            // TODO seperate out exceptions
            System.out.println("An error occured:  " + e);
    ...
    /**
     * Given a set of codes and a state returns a HashMap of the average
     * submitted charge amount for each code
     * 
     * @param codes
     *            set of hcpcs_codes
     * @param state
     *            2 letter upper case state abbreviation
     * @return HashMap<String, Double> containing the average submitted charge
     *         amount for each code. will contain -1 for any codes not found in
     *         the table and 0 if query fails.
     */
     ...
     * NEW USE CASE #2 Provides the Top N treatments that have the
     * largest/smallest gap between cost of treatment and what Medicare pays
     * starting at the query result for given index (for paging)
     * 
     * @author Tim and others
     * 
     * @param largest
     *            true to return in descending order from largest to smallest,
     *            otherwise from smallest to largest
     * @param start
     *            the index of the first result to return from the query result
     * @param numReturn
     *            the top N to return
     * @return List<ProcedureDetails>
     */
     ...
     * NEW USE CASE #3 Provide the Top N treatments that have the
     * largest/smallest gap between cost of treatment and what Medicare pays in
     * terms of percentage (the percent the patient is responsible for) starting
     * at the query result for given index (for paging)
     * 
     * @author Tim and others
     * 
     * @param largest
